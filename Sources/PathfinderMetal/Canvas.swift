...
public class Canvas {
  ...
  enum FillStyle {
    ...
    func into_paint() -> Paint1 {
      switch self {
      case .color(let color):
        return Paint1(color: color)
  ...
  struct State {
    ...
    var fill_paint: Paint1
    var stroke_paint: Paint1
  ...
  private func build(options: Scene1.BuildOptions) -> AsyncStream<RenderCommand1> {
    AsyncStream { continuation in
      Task {
        var listener = SceneBuilder1.RenderCommandListener(send_fn: { cmd in
        ...
        var sink = SceneBuilder1.SceneSink(listener)
  ...
  public func demo(device: MTLDevice, drawable: CAMetalDrawable, size: SIMD2<Int32>) {
    ...
    func addEllipse(_ cx: Float, _ cy: Float, _ rx: Float, _ ry: Float) -> PFPath {
      ...
      path.bezier_curve_to(
        .init(x: cx - rx, y: cy - oy), .init(x: cx - ox, y: cy - ry), .init(x: cx, y: cy - ry))
      path.bezier_curve_to(
        .init(x: cx + ox, y: cy - ry), .init(x: cx + rx, y: cy - oy), .init(x: cx + rx, y: cy))
      path.bezier_curve_to(
        .init(x: cx + rx, y: cy + oy), .init(x: cx + ox, y: cy + ry), .init(x: cx, y: cy + ry))
      path.bezier_curve_to(
        .init(x: cx - ox, y: cy + ry), .init(x: cx - rx, y: cy + oy), .init(x: cx - rx, y: cy))
    ...
    func makeEllipseOutline(cx: Float, cy: Float, rx: Float, ry: Float, reverse: Bool = false)
      -> PFPath
    {
      var p = PFPath()
    ...
    func hachureRect(
      x: Float, y: Float, w: Float, h: Float, gap: Float = 10, angle: Float = .pi / 12,
      strokeWidth: Float = 3
    ) -> (PFPath, PFPath, PFPath) {
      // Inset to keep hatches away from the border by half the border width
      let inset = max(1, strokeWidth * 0.75)
      let minX = x + inset
      let maxX = x + w - inset
      let minY = y + inset
      let maxY = y + h - inset
    ...
    var (hachurePath, border, hachureClip) = hachureRect(
      x: 120, y: 800, w: 240, h: 190, gap: 10, angle: .pi / 6, strokeWidth: 4)
  ...
  public func render(
    device: PFDevice, options: Scene1.BuildOptions, size: SIMD2<Int32>, backgroundColor: ColorF?
  ) {
    if renderer == nil {
      renderer = Renderer(
        device: device,
  ...
  func clear_rect(_ rect: PFRect<Float32>) {
    ...
    let paint = Paint1.transparent_black
    ...
    var draw_path = Scene1.DrawPath(outline, paint_id)
    draw_path.blend_mode = .clear
    _ = scene.push_draw_path(draw_path)
  }

  func fill_path(_ path: PFPath, _ fill_rule: Scene1.FillRule) {
    var path = path
    var outline = path.into_outline()
  ...
  func stroke_path(_ path: PFPath) {
    ...
    var stroke_to_fill = Outline1.OutlineStrokeToFill(outline, style: stroke_style)
  ...
  func clip_path(_ path: inout PFPath, _ fill_rule: Scene1.FillRule) {
    var outline = path.into_outline()
    outline.transform(current_state.transform)

    var clip_path = Scene1.ClipPath(outline)
  ...
  func push_path(_ outline: inout Outline1, _ path_op: PathOp, _ fill_rule: Scene1.FillRule) {
    ...
    if !current_state.shadow_color.isFullyTransparent {
      ...
      let shadow_blur_info = Self.push_shadow_blur_render_targets_if_needed(
        &scene, current_state, shadow_outline.bounds)
      ...
      var shadow_path = Scene1.DrawPath(shadow_outline, shadow_paint_id)
    ...
    var path = Scene1.DrawPath(outline, paint_id)
  ...
  static func composite_shadow_blur_render_targets_if_needed(
    _ scene: inout Scene1,
    _ info: ShadowBlurRenderTargetInfo?,
    _ clip_path: UInt32?
  ) {
    guard let info = info else { return }

    var paint_x = Pattern1(id: info.id_x, size: info.bounds.size)
    var paint_y = Pattern1(id: info.id_y, size: info.bounds.size)
    ...
    let paint_id_x = scene.push_paint(Paint1(pattern: paint_x))
    let paint_id_y = scene.push_paint(Paint1(pattern: paint_y))

    // TODO(pcwalton): Apply clip as necessary.
    let outline_x = Outline1(
      rect: .init(origin: SIMD2<Float>(0.0, 0.0), size: info.bounds.f32.size))
    let path_x = Scene1.DrawPath(outline_x, paint_id_x)
    let outline_y = Outline1(rect: info.bounds.f32)
    var path_y = Scene1.DrawPath(outline_y, paint_id_y)
  ...
  static func push_shadow_blur_render_targets_if_needed(
    ...
    let render_target_y = Scene1.RenderTarget(size: bounds.size, name: "")
    let render_target_id_y = scene.push_render_target(render_target_y)
    let render_target_x = Scene1.RenderTarget(size: bounds.size, name: "")
...
extension Canvas.State {
  ...
  func resolve_paint(_ paint: Paint1) -> Paint1 {
...
struct ShadowBlurRenderTargetInfo {
  var id_x: Scene1.RenderTargetId
  var id_y: Scene1.RenderTargetId
