...
struct Contour1 {
  ...
  mutating func push_arc(
    _ transform: Transform, _ start_angle: Float, _ end_angle: Float,
  ...
  mutating func push_arc_from_unit_chord(
    _ transform: Transform, _ chord: LineSegment, _ direction: PFPath.ArcDirection
  ) {
    var chord = chord
  ...
  mutating func push_ellipse(_ transform: Transform) {
    let segment = Segment1.quarter_circle_arc
  ...
  mutating func push_segment(_ segment: Segment1, _ flags: PushSegmentFlags) {
    if segment.isNone {
      return
    }
  ...
  static func union_rect(
    _ bounds: inout PFRect<Float32>, _ new_point: SIMD2<Float32>, _ first: Bool
  ) {
    if first {
...
struct ContourIter {
  private let contour: Contour1
  private var index: Int
  private let flags: Contour1.ContourIterFlags

  init(contour: Contour1, index: Int, flags: Contour1.ContourIterFlags) {
...
extension ContourIter: IteratorProtocol {
  typealias Element = Segment1

  mutating func next() -> Segment1? {
    ...
    if index == contour.len() {
      ...
      index += 1
      return Segment1(line: .init(from: point0, to: point1))
    }
    ...
    if contour.point_is_endpoint(point2_index) {
      return Segment1(quadratic: .init(from: point0, to: point2), ctrl: point1)
    }
    ...
    return Segment1(cubic: .init(from: point0, to: point3), ctrl: .init(from: point1, to: point2))
  }
}
...
struct ContourDash {
  ...
  init(input: Contour1, output: Outline1, state: DashState) {
    self.input = input
    self.output = output
    self.state = state
  }
  ...
  mutating func dash() {
    ...
    var queued_segment: Segment1? = nil
...
extension Contour1.ContourStrokeToFill {
  init(_ input: Contour1, _ output: Contour1, _ radius: Float, _ join: Canvas.StrokeStyle.LineJoin)
  {
    self.input = input
    self.output = output
    self.radius = radius
