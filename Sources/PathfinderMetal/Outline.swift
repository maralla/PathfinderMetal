...
struct Outline1 {
  ...
  init(segments: [Segment1]) {
    var currentContour = Contour1()
  ...
  init(rect: PFRect<Float32>) {
    pushContour(Contour1(rect: rect))
  }

  init(rect: PFRect<Float32>, radius: SIMD2<Float32>) {
    pushContour(Contour1(rect: rect, radius: radius))
  }

  /// Adds a new subpath to this outline.
  mutating func pushContour(_ contour: Contour1) {
    if contour.isEmpty {
      return
    }
  ...
  mutating func popContour() -> Contour1? {
...
struct OutlineDash {
  ...
  init(_ input: Outline1, _ dashes: [Float], _ offset: Float) {
    self.input = input
    self.output = Outline1()
  ...
  mutating func into_outline() -> Outline1 {
    if state.is_on() {
...
extension Outline1.OutlineStrokeToFill {
  /// Creates a new `OutlineStrokeToFill` object that will stroke the given outline with the
  /// given stroke style.
  init(_ input: Outline1, style: Canvas.StrokeStyle) {
    self.input = input
    self.output = Outline1()
    self.style = style
  }
  ...
  mutating func offset() {
    var new_contours: [Contour1] = []
    for input in self.input.contours {
      let closed = input.closed
      var stroker = Contour1.ContourStrokeToFill(
        input,
        Contour1(),
      ...
      if closed {
        ...
        stroker = Contour1.ContourStrokeToFill(
          input,
          Contour1(),
          self.style.line_width * 0.5,
          self.style.line_join)
  ...
  func into_outline() -> Outline1 {
    return output
  }

  private mutating func push_stroked_contour(
    _ new_contours: inout [Contour1],
    _ stroker: inout Contour1.ContourStrokeToFill,
    _ closed: Bool
  ) {
  ...
  private mutating func add_cap(_ contour: inout Contour1) {
    ...
    repeat {
      ...
      if simd.length_squared(p1 - p0) > Contour1.EPSILON {
        break
      }
      if p0_index == 0 {
        return
