struct Paint: Hashable {
    ...
    enum PaintFilter {
        ...
        case patternFilter(Pattern.PatternFilter)
    }
    ...
    struct PaintColorTextureMetadata {
        ...
        var tile_batch_texture: RenderCommand.TileBatchTexture {
            .init(
    ...
    static var transparent_black: Paint {
...
struct Palette1 {
    ...
    struct GradientTileBuilder {
        ...
        func create_render_commands(render_commands: inout [RenderCommand]) {
            for tile in tiles {
    ...
    struct PaintLocationsInfo {
        var paint_metadata: [Paint.PaintMetadata]
    ...
    var paints: [Paint]
    var render_targets: [Scene.RenderTarget]
    var cache: [Paint: UInt16]
    var scene_id: UInt32
}

extension Palette1.GradientTileBuilder {
    mutating func allocate(
        allocator: inout TextureAllocator,
        transient_paint_locations: inout [SceneBuilder.TextureLocation],
        gradient: Gradient
    ) -> SceneBuilder.TextureLocation {
        if self.tiles.isEmpty || self.tiles.last!.next_index == Gradient.GRADIENT_TILE_LENGTH {
            let size = SIMD2<Int32>(repeating: Int32(Gradient.GRADIENT_TILE_LENGTH))
...
extension Palette1 {
    ...
    mutating func push_render_target(_ render_target: Scene.RenderTarget) -> Scene.RenderTargetId {
    ...
    func build_paint_info(
        texture_manager: inout SceneBuilder.PaintTextureManager,
        render_transform: Transform
    ) -> Paint.PaintInfo {
        // Assign render target locations.
        var transient_paint_locations: [SceneBuilder.TextureLocation] = []
        ...
        var render_commands = [RenderCommand.uploadTextureMetadata(texture_metadata)]
    ...
    func assign_paint_locations(
        ...
        _ transient_paint_locations: inout [SceneBuilder.TextureLocation]
        ...
        for paint in paints {
            var color_texture_metadata: Paint.PaintColorTextureMetadata? = nil
            if let overlay = paint.overlay {
                ...
                case .pattern(let pattern):
                    ...
                    case .image(let image):
                        ...
                        if let cached_location = texture_manager.cached_images[image_hash] {
                            ...
                            location = texture_manager.allocator.allocate(
                                requested_size: image.size &+ border &* 2,
                                mode: .ownPage
                            )
                    ...
                    var filter = Paint.PaintFilter.none
    ...
    func calculate_texture_transforms(
        _ paint_metadata: inout [Paint.PaintMetadata],
        _ texture_manager: SceneBuilder.PaintTextureManager,
        ...
        .TextureMetadataEntry]
    {
        paint_metadata.map({ paint_metadata in
            ...
            var combineMode = RenderCommand.ColorCombineMode.none
    ...
    func create_render_commands(
        ...
        for (index, metadata) in render_target_metadata.enumerated() {
            let id = Scene.RenderTargetId(scene: self.scene_id, render_target: UInt32(index))
    ...
    func free_transient_locations(
        _ texture_manager: inout SceneBuilder.PaintTextureManager,
        _ transient_paint_locations: [SceneBuilder.TextureLocation]
    ) {
