...
struct PFPath {
  ...
  var current_contour: Contour1

  init() {
    outline = Outline1()
    current_contour = Contour1()
  }
  ...
  mutating func bezier_curve_to(
    _ ctrl0: SIMD2<Float32>, _ ctrl1: SIMD2<Float32>, _ to: SIMD2<Float32>
  ) {
    current_contour.pushCubic(ctrl0: ctrl0, ctrl1: ctrl1, to: to)
  }

  mutating func arc(
    _ center: SIMD2<Float32>, _ radius: Float, _ start_angle: Float, _ end_angle: Float,
  ...
  mutating func ellipse(
    _ center: SIMD2<Float32>, _ axes: SIMD2<Float32>, _ rotation: Float, _ start_angle: Float,
    _ end_angle: Float
  ) {
    flush_current_contour()
