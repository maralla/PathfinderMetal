...
enum RenderCommand1 {
  ...
  struct TileBatchTexture: Equatable {
    ...
    var composite_op: Paint1.PaintCompositeOp
  }
  ...
  struct TileBatchDataD3D11 {
    ...
    init(_ batch_id: UInt32, _ mode: SceneBuilder1.PrepareMode, _ path_source: PathSource) {
  ...
  case uploadTexelData(texels: [ColorU], location: SceneBuilder1.TextureLocation)

  // Associates a render target with a texture page.
  //
  // TODO(pcwalton): Add a rect to this so we can render to subrects of a page.
  case declareRenderTarget(id: Scene1.RenderTargetId, location: SceneBuilder1.TextureLocation)
  ...
  // stack.
  case pushRenderTarget(Scene1.RenderTargetId)
...
extension RenderCommand1.SegmentsD3D11 {
  mutating func add_path(outline: Outline1) -> Range<UInt32> {
      ...
      for point_index in 0..<point_count {
        if contour.flags_of(point_index).intersection([.controlPoint0, .controlPoint1]).isEmpty {
            ...
            && contour.flags_of(point_index + 1).contains(.controlPoint0)
          {
            ...
            {
              flags = RenderCommand1.CURVE_IS_CUBIC
            } else {
              flags = RenderCommand1.CURVE_IS_QUADRATIC
            }
          }
...
extension RenderCommand1.TileBatchDataD3D11 {
  func init_backdrops(
    _ backdrops: inout [RenderCommand1.BackdropInfoD3D11],
    _ path_index: UInt32,
  ...
  mutating func push(
    _ path: SceneBuilder1.BuiltPath,
    _ global_path_id: UInt32,
    _ batch_clip_path_id: SceneBuilder1.GlobalPathId?,
    _ z_write: Bool,
