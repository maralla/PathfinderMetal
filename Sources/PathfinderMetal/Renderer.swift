...
struct RendererCore {
  ...
  static let MASK_FRAMEBUFFER_WIDTH: Int32 =
    Int32(SceneBuilder1.TILE_WIDTH) * Int32(MASK_TILES_ACROSS)
  static let MASK_FRAMEBUFFER_HEIGHT: Int32 =
    Int32(SceneBuilder1.TILE_HEIGHT) / 4 * Int32(MASK_TILES_DOWN)
  ...
  var render_target_stack: [Scene1.RenderTargetId]
  ...
  func render_target_location(_ render_target_id: Scene1.RenderTargetId)
  ...
  func tile_size() -> SIMD2<Int32> {
    let temp =
      draw_viewport().size
      &+ .init(Int32(SceneBuilder1.TILE_WIDTH) - 1, Int32(SceneBuilder1.TILE_HEIGHT) - 1)
    return .init(
      temp.x / Int32(SceneBuilder1.TILE_WIDTH), temp.y / Int32(SceneBuilder1.TILE_HEIGHT))
  }

  func pixel_size_to_tile_size(_ pixel_size: SIMD2<Int32>) -> SIMD2<Int32> {
    // Round up.
    let tile_size = SIMD2<Int32>(
      Int32(SceneBuilder1.TILE_WIDTH) - 1, Int32(SceneBuilder1.TILE_HEIGHT) - 1)
    let size = pixel_size &+ tile_size
    return .init(
      size.x / Int32(SceneBuilder1.TILE_WIDTH), size.y / Int32(SceneBuilder1.TILE_HEIGHT))
  }
...
struct VertexArraysCore {
  ...
  init(
    _ device: PFDevice, _ programs: ProgramsCore,
    _ quad_vertex_positions_buffer: PFDevice.Buffer,
    _ quad_vertex_indices_buffer: PFDevice.Buffer
  ) {
...
struct RendererD3D11 {
  ...
  struct SceneSourceBuffers {
    ...
    mutating func upload(
      ...
      _ device: PFDevice,
      _ segments: RenderCommand1.SegmentsD3D11
    ) {
      ...
      if point_indices_capacity < needed_point_indices_capacity {
        point_indices_buffer = allocator.allocate_general_buffer(
          ...
          Int(needed_point_indices_capacity)
            * MemoryLayout<RenderCommand1.SegmentIndicesD3D11>.stride,
          "PointIndicesD3D11"
        )
  ...
  struct SceneBuffers {
    ...
    mutating func upload(
      _ allocator: inout GPUMemoryAllocator,
      _ device: PFDevice,
      _ draw_segments: RenderCommand1.SegmentsD3D11,
      _ clip_segments: RenderCommand1.SegmentsD3D11
    ) {
  ...
  struct DiceProgramD3D11 {
    ...
    init(_ device: PFDevice) {
      ...
      let compute_indirect_params_storage_buffer = device.get_storage_buffer(
        program, "ComputeIndirectParams", 0)
  ...
  struct FillProgramD3D11 {
    ...
    init(_ device: PFDevice) {
      ...
      let local_size = ComputeDimensions(
        x: SceneBuilder1.TILE_WIDTH, y: SceneBuilder1.TILE_HEIGHT / 4, z: 1)
      device.set_compute_program_local_size(&program, local_size)
...
extension GPUMemoryAllocator {
  ...
  mutating func allocate_general_buffer(_ device: PFDevice, _ size: Int, _ tag: String) -> UInt64 {
    ...
    device.allocate_buffer(
      &buffer, PFDevice.BufferData<UInt8>.uninitialized(Int(byte_size)), .vertex)
  ...
  mutating func allocate_index_buffer(_ device: PFDevice, _ size: Int, _ tag: String) -> UInt64 {
    ...
    var buffer = device.create_buffer(.dynamic)
    device.allocate_buffer(
      &buffer, PFDevice.BufferData<UInt8>.uninitialized(Int(byte_size)), .index)
...
extension Renderer {
  ...
  init(device: PFDevice, options: RendererOptions) {
    ...
    let quad_vertex_positions_buffer_id = allocator.allocate_general_buffer(
      device, Renderer.QUAD_VERTEX_POSITIONS.count * MemoryLayout<UInt16>.stride,
      "QuadVertexPositions")
    ...
    let quad_vertex_indices_buffer_id = allocator.allocate_index_buffer(
      device, Renderer.QUAD_VERTEX_INDICES.count * MemoryLayout<UInt32>.stride, "QuadVertexIndices")
    ...
    let area_lut_texture_id = allocator.allocate_texture(
      device, SIMD2<Int32>(repeating: 256), .rgba8, "AreaLUT")
    let gamma_lut_texture_id = allocator.allocate_texture(
      device, SIMD2<Int32>(256, 8), .r8, "GammaLUT")
    ...
    let intermediate_dest_framebuffer_id = allocator.allocate_framebuffer(
      device, window_size, .rgba8, "IntermediateDest")
    ...
    let texture_metadata_texture_id = allocator.allocate_texture(
      device, texture_metadata_texture_size, .rgba16F, "TextureMetadata")
    ...
    if core.intermediate_dest_framebuffer_size != main_viewport.size {
      ...
      core.intermediate_dest_framebuffer_id =
        core.allocator.allocate_framebuffer(
          core.device, main_viewport.size, .rgba8, "IntermediateDest")
  ...
  mutating func render_command(command: RenderCommand1) {
    switch command {
  ...
  mutating func allocate_pattern_texture_page(
    _ page_id: UInt32,
    _ descriptor: RenderCommand1.TexturePageDescriptor
  ) {
    ...
    let framebuffer_id = self.core.allocator.allocate_framebuffer(
      self.core.device, texture_size, .rgba8, "PatternPage")
  ...
  mutating func upload_texel_data(_ texels: [ColorU], _ location: SceneBuilder1.TextureLocation) {
  ...
  mutating func declare_render_target(
    _ render_target_id: Scene1.RenderTargetId,
    _ location: SceneBuilder1.TextureLocation
  ) {
  ...
  func compute_filter_params(
    _ filter: Filter,
    _ blend_mode: Scene1.BlendMode,
    _ color_0_combine_mode: RenderCommand1.ColorCombineMode
  ) -> FilterParams {
    var ctrl: Int32 = 0
    ...
    case .patternFilter(.colorMatrix(let matrix)):
      return .init(
        p0: matrix.f1, p1: matrix.f2, p2: matrix.f3, p3: matrix.f4, p4: matrix.f5,
        ctrl: ctrl
  ...
  mutating func upload_texture_metadata(_ metadata: [RenderCommand1.TextureMetadataEntry]) {
  ...
  mutating func push_render_target(_ render_target_id: Scene1.RenderTargetId) {
...
extension RendererD3D11 {
  ...
  mutating func prepare_tiles(
    _ core: inout RendererCore,
    _ batch: RenderCommand1.TileBatchDataD3D11
  ) {
  ...
  mutating func upload_propagate_metadata(
    _ core: inout RendererCore,
    _ propagate_metadata: [RenderCommand1.PropagateMetadataD3D11],
    _ backdrops: [RenderCommand1.BackdropInfoD3D11]
  ) -> PropagateMetadataBufferIDsD3D11 {
    let propagate_metadata_storage_id = core.allocator.allocate_general_buffer(
      core.device,
      propagate_metadata.count * MemoryLayout<RenderCommand1.PropagateMetadataD3D11>.stride,
    ...
    let backdrops_storage_id = core.allocator.allocate_general_buffer(
      core.device,
      backdrops.count * MemoryLayout<RenderCommand1.BackdropInfoD3D11>.stride,
      "BackdropInfoD3D11"
    )
  ...
  mutating func dice_segments(
    _ core: inout RendererCore,
    _ dice_metadata: [RenderCommand1.DiceMetadataD3D11],
    _ batch_segment_count: UInt32,
    _ path_source: RenderCommand1.PathSource,
    ...
    let microlines_buffer_id = core.allocator.allocate_general_buffer(
      core.device,
      Int(allocated_microline_count) * MemoryLayout<RenderCommand1.MicrolineD3D11>.stride,
      "MicrolineD3D11"
    )
    let dice_metadata_buffer_id = core.allocator.allocate_general_buffer(
      core.device,
      dice_metadata.count * MemoryLayout<RenderCommand1.DiceMetadataD3D11>.stride,
      "DiceMetadataD3D11"
    )
  ...
  mutating func bound(
    ...
    _ tile_count: UInt32,
    _ tile_path_info: [RenderCommand1.TilePathInfoD3D11]
  ) {
    let bound_program = programs.bound_program

    let path_info_buffer_id = core.allocator.allocate_general_buffer(
      core.device,
      tile_path_info.count * MemoryLayout<RenderCommand1.TilePathInfoD3D11>.stride,
      "TilePathInfoD3D11"
    )
  ...
  mutating func propagate_tiles(
    ...
    var zBuffer = core.allocator.general_buffers_in_use[z_buffer_id]!
    core.device.upload_to_buffer(
      &zBuffer.buffer, 0, Array(repeating: Int32(0), count: tile_area), .storage)
    core.allocator.general_buffers_in_use[z_buffer_id] = zBuffer
  ...
  mutating func prepare_and_draw_tiles(
    _ core: inout RendererCore,
    _ batch: RenderCommand1.DrawTileBatchD3D11
  ) {
  ...
  mutating func draw_tiles(
    ...
    _ tiles_d3d11_buffer_id: UInt64,
    _ first_tile_map_buffer_id: UInt64,
    _ color_texture_0: RenderCommand1.TileBatchTexture?
  ) {
    ...
    uniforms.append(
      (
        ...
        .vec2(SIMD2<Float>(Float(SceneBuilder1.TILE_WIDTH), Float(SceneBuilder1.TILE_HEIGHT)))
      ))
    uniforms.append(
      (
    ...
    uniforms.append(
      (
        ...
        .ivec2(
          SIMD2<Int32>(
            Renderer.TEXTURE_METADATA_TEXTURE_WIDTH, Renderer.TEXTURE_METADATA_TEXTURE_HEIGHT))
      ))
