...
public struct Scene1 {
  ...
  struct DrawPath {
    ...
    init(_ outline: Outline1, _ paint: UInt16) {
  ...
  public struct BuildOptions {
    ...
    func prepare(_ bounds: PFRect<Float32>) -> SceneBuilder1.PreparedBuildOptions {
      let renderTransform: SceneBuilder1.PreparedRenderTransform
...
struct SceneBuilder1 {
  ...
  typealias RenderCommandSendFunction = (RenderCommand1) -> Void

  struct RenderCommandListener {
    var send_fn: RenderCommandSendFunction

    func send(_ render_command: RenderCommand1) {
  ...
  struct LastSceneInfo {
    var scene_id: UInt32
    var scene_epoch: Scene1.SceneEpoch
  ...
  struct PaintTextureManager {
    var allocator: TextureAllocator1
    var cached_images: [UInt64: TextureLocation]

    init() {
      self.allocator = TextureAllocator1()
      self.cached_images = [:]
    }
  }
  ...
  struct BuiltSegments {
    var draw_segments: RenderCommand1.SegmentsD3D11
    var clip_segments: RenderCommand1.SegmentsD3D11
    var draw_segment_ranges: [Range<UInt32>]
    var clip_segment_ranges: [Range<UInt32>]
  }
  ...
  struct ClipBatchesD3D11 {
    ...
    var prepare_batches: [RenderCommand1.TileBatchDataD3D11]
  ...
  struct TileBatchBuilder {
    ...
    var prepare_commands: [RenderCommand1]
    var draw_commands: [RenderCommand1]
    ...
    func send_to(sink: SceneBuilder1.SceneSink) {
  ...
  struct BuiltDrawPath {
    ...
    var filter: Filter
    var color_texture: RenderCommand1.TileBatchTexture?
    var sampling_flags_1: RenderCommand1.TextureSamplingFlags
    var mask_0_fill_rule: Scene1.FillRule
    var occludes: Bool

    init(
      _ built_path: BuiltPath, _ path_object: Scene1.DrawPath,
      _ paint_metadata: Paint1.PaintMetadata
    ) {
  ...
  enum TilingPathInfo {
    ...
    var ctrl: UInt8 {
      ...
      case .draw(let draw_tiling_path_info):
        ...
        case .evenOdd:
          ctrl |= UInt8(
            SceneBuilder1.TILE_CTRL_MASK_EVEN_ODD << SceneBuilder1.TILE_CTRL_MASK_0_SHIFT)
        case .winding:
          ctrl |= UInt8(
            SceneBuilder1.TILE_CTRL_MASK_WINDING << SceneBuilder1.TILE_CTRL_MASK_0_SHIFT)
        }
      case .clip:
        break
      }
  ...
  struct PreparedClipPath {
    var built_path: SceneBuilder1.BuiltPath
    var subclip_id: GlobalPathId?
  }

  var scene: Scene1
  var built_options: SceneBuilder1.PreparedBuildOptions
  var next_alpha_tile_indices: [Int]  // length 2
  var sink: SceneSink
}

extension SceneBuilder1.BuiltPath {
  static func round_rect_out_to_tile_bounds(_ rect: PFRect<Float32>) -> PFRect<Int32> {
    (rect
      * SIMD2<Float32>(
        1.0 / Float32(SceneBuilder1.TILE_WIDTH), 1.0 / Float32(SceneBuilder1.TILE_HEIGHT)))
      .round_out().i32
  }
  ...
  init(
    ...
    _ fill_rule: Scene1.FillRule,
    _ prepare_mode: SceneBuilder1.PrepareMode,
    _ clip_path_id: UInt32?,
    _ tiling_path_info: SceneBuilder1.TilingPathInfo
  ) {
...
extension SceneBuilder1.TileBatchBuilder {
  func prepare_draw_path_for_gpu_binning(
    _ scene: Scene1,
    _ built_options: SceneBuilder1.PreparedBuildOptions,
    _ draw_path_id: UInt32,
    _ prepare_mode: SceneBuilder1.PrepareMode,
    _ paint_metadata: [Paint1.PaintMetadata]
  ) -> SceneBuilder1.BuiltDrawPath? {
    ...
    let built_path = SceneBuilder1.BuiltPath(
      draw_path_id,
      path_bounds,
      effective_view_box,
      draw_path.fill_rule,
  ...
  private func fixup_batch_for_new_path_if_possible(
    _ batch_color_texture: inout RenderCommand1.TileBatchTexture?,
    _ draw_path: SceneBuilder1.BuiltDrawPath
  ) -> Bool {
  ...
  mutating func build_tile_batches_for_draw_path_display_item(
    _ scene: Scene1,
    _ sink: SceneBuilder1.SceneSink,
    _ built_options: SceneBuilder1.PreparedBuildOptions,
    _ draw_path_id_range: Range<UInt32>,
    _ paint_metadata: [Paint1.PaintMetadata],
    _ prepare_mode: SceneBuilder1.PrepareMode
  ) {
    var draw_tile_batch: RenderCommand1.DrawTileBatchD3D11? = nil
    for draw_path_id in draw_path_id_range {
      ...
      if var existing_batch = draw_tile_batch {
        flush_needed = !fixup_batch_for_new_path_if_possible(
          &existing_batch.color_texture, draw_path)
      ...
      let clip_path: SceneBuilder1.GlobalPathId?
  ...
  func add_clip_path_to_batch(
    ...
    _ prepare_mode: SceneBuilder1.PrepareMode,
    _ clip_level: Int,
    _ clip_batches_d3d11: inout SceneBuilder1.ClipBatchesD3D11
  ) -> SceneBuilder1.GlobalPathId? {
    ...
    let clip_path_batch_index = clip_batches_d3d11.prepare_batches[clip_level].push(
      clip_path, clip_path_id, subclip_id, true, sink)
  ...
  func prepare_clip_path_for_gpu_binning(
    ...
    _ clip_level: Int,
    _ clip_batches_d3d11: inout SceneBuilder1.ClipBatchesD3D11
  ) -> SceneBuilder1.PreparedClipPath {
    let transform = prepare_mode.transform
...
extension Scene1 {
  ...
  mutating func build(options: BuildOptions, sink: inout SceneBuilder1.SceneSink) {
    let prepared_options = options.prepare(self.bounds)

    var builder = SceneBuilder1(self, prepared_options, sink)
    builder.build()
  ...
  func build_paint_info(
    _ texture_manager: inout SceneBuilder1.PaintTextureManager, _ render_transform: Transform
  ) -> Paint1.PaintInfo {
    palette.build_paint_info(texture_manager: &texture_manager, render_transform: render_transform)
  }

  func effective_view_box(_ render_options: SceneBuilder1.PreparedBuildOptions) -> PFRect<Float32> {
  ...
  mutating func push_render_target(_ render_target: RenderTarget) -> Scene1.RenderTargetId {
...
extension SceneBuilder1.BuiltSegments {
  init(scene: Scene1) {
    draw_segments = RenderCommand1.SegmentsD3D11()
    clip_segments = RenderCommand1.SegmentsD3D11()
    draw_segment_ranges = []
    clip_segment_ranges = []
...
extension Canvas.CompositeOperation {
  func to_blend_mode() -> Scene1.BlendMode {
    switch self {
