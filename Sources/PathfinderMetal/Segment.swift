...
struct LineSegment: Hashable {
  ...
  func offset(_ distance: Float) -> LineSegment {
    if self.is_zero_length() {
      return self
    } else {
      return self + normalize(SIMD2<Float32>(vector.y, vector.x))
        * SIMD2<Float32>(-distance, distance)
    }
  }
...
extension Segment1 {
  ...
  init(arcFromCos cos_sweep_angle: Float) {
    ...
    if cos_sweep_angle >= 1.0 - Contour1.EPSILON {
    ...
    let p3p0 =
      SIMD4<Float32>(
        sqrt(intermediate.x), sqrt(intermediate.y), sqrt(intermediate.z), sqrt(intermediate.w))
      * signs
  ...
  /// axis.
  static var quarter_circle_arc: Segment1 {
    let p0 = SIMD2<Float32>(repeating: Contour1.SQRT_2 * 0.5)
    let p1 = SIMD2<Float32>(
      -Contour1.SQRT_2 / 6.0 + 4.0 / 3.0, 7.0 * Contour1.SQRT_2 / 6.0 - 4.0 / 3.0)
  ...
  func split(_ t: Float) -> (Segment1, Segment1) {
    // FIXME(pcwalton): Don't degree elevate!
    if is_line() {
      let (before, after) = as_line_segment().split(t)
      return (Segment1(line: before), Segment1(line: after))
    } else {
  ...
  func transform(_ transform: Transform) -> Segment1 {
    return .init(
...
struct CubicSegment {
  ...
  func split_before(_ t: Float) -> Segment1 {
    return split(t).0
  }

  /// A convenience method equivalent to `segment.split(t).1`.
  func split_after(_ t: Float) -> Segment1 {
    return split(t).1
  }
...
extension Segment1 {
  ...
  func offset(_ distance: Float, _ join: Canvas.StrokeStyle.LineJoin, _ contour: inout Contour1) {
  ...
  func add_to_contour(
    ...
    if contour.might_need_join(join) {
      ...
      contour.add_join(distance, join, join_point, LineSegment(from: p4, to: p3))
    }

    // Push segment.
    contour.push_segment(self, [.UPDATE_BOUNDS, .INCLUDE_FROM_POINT])
  }

  func offset_once(_ distance: Float) -> Segment1 {
    if self.is_line() {
      return Segment1(line: self.baseline.offset(distance))
    }

    if self.is_quadratic() {
      var segment_0 = LineSegment(from: self.baseline.from, to: self.ctrl.from)
      var segment_1 = LineSegment(from: self.ctrl.from, to: self.baseline.to)
      ...
      return Segment1(quadratic: baseline, ctrl: ctrl)
    }

    assert(self.is_cubic())

    if self.baseline.from == self.ctrl.from {
      var segment_0 = LineSegment(from: self.baseline.from, to: self.ctrl.to)
      var segment_1 = LineSegment(from: self.ctrl.to, to: self.baseline.to)
      segment_0 = segment_0.offset(distance)
      segment_1 = segment_1.offset(distance)
      let ctrl: SIMD2<Float32>
      if let t = segment_0.intersection_t(segment_1) {
      ...
      return Segment1(cubic: baseline, ctrl: ctrl_segment)
    }
    ...
    var segment_0 = LineSegment(from: self.baseline.from, to: self.ctrl.from)
    var segment_1 = LineSegment(from: self.ctrl.from, to: self.ctrl.to)
    var segment_2 = LineSegment(from: self.ctrl.to, to: self.baseline.to)
    segment_0 = segment_0.offset(distance)
    segment_1 = segment_1.offset(distance)
    segment_2 = segment_2.offset(distance)
    let (ctrl_0, ctrl_1): (SIMD2<Float32>, SIMD2<Float32>)
    ...
    return Segment1(cubic: baseline, ctrl: ctrl)
  }

  func error_is_within_tolerance(_ other: Segment1, _ distance: Float) -> Bool {
    let SAMPLE_COUNT: UInt32 = 16

    var min = abs(distance) - Self.TOLERANCE
    var max = abs(distance) + Self.TOLERANCE
    min = min <= 0.0 ? 0.0 : min * min
    max = max <= 0.0 ? 0.0 : max * max

    for t_num in 0...(SAMPLE_COUNT) {
      let t = Float(t_num) / Float(SAMPLE_COUNT)
