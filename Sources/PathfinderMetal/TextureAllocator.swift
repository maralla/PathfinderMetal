struct TextureAllocator {
    ...
    struct TexturePageIter: Sequence, IteratorProtocol {
        ...
        let allocator: TextureAllocator
        var nextIndex: Int
...
extension TextureAllocator.TreeNode {
    mutating func allocate(
        this_origin: SIMD2<Int32>,
        this_size: UInt32,
    ...
    mutating func free(
        _ this_origin: SIMD2<Int32>,
        _ this_size: UInt32,
        _ requested_origin: SIMD2<Int32>,
    ...
    mutating func merge_if_necessary() {
        if case .parent(let k1, let k2, let k3, let k4) = self {
...
extension TextureAllocator.TextureAtlasAllocator {
    init() {
        self.init(length: TextureAllocator.ATLAS_TEXTURE_LENGTH)
    }
...
extension TextureAllocator {
    init() {
        self.pages = []
    }

    mutating func allocate(
        requested_size: SIMD2<Int32>,
        mode: AllocationMode
    )
        -> SceneBuilder.TextureLocation
    {
        // If requested, or if the image is too big, use a separate page.
        if mode == .ownPage || requested_size.x > Int32(TextureAllocator.ATLAS_TEXTURE_LENGTH)
            || requested_size.y > Int32(TextureAllocator.ATLAS_TEXTURE_LENGTH)
        {
            ...
            if let page {
                ...
                case .atlas(var allocator):
                    if let rect = allocator.allocate(requested_size: requested_size) {
                        ...
                        return SceneBuilder.TextureLocation(page: UInt32(page_index), rect: rect)
                    }
                }
            }
        }
    ...
    mutating func free(location: SceneBuilder.TextureLocation) {
